...
class SearchWord extends StatefulWidget {
  ...
  const SearchWord({super.key, required this.courseName,});
...
class _SearchWordState extends State<SearchWord> {
  Future<List<Map<String, dynamic>>> hskFuture = Future<List<Map<String, dynamic>>>.value([]);
  Future<List<Map<String, dynamic>>> unitsFuture = SQLHelper.getCourseUnits("hsk");
  ...
  void addSelectedWordToUnit({required String hanzi, required String pinyin, required String translations0,}){
    setState(() {
      SQLHelper.addWordToCustomCourse(
          course: widget.courseName, hanzi: hanzi, pinyin: pinyin, translations0: translations0,
      );
    });
  }
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: const CupertinoNavigationBar(middle: Text("Search"),),
        body: Column(
          children: [
            Padding(
                ...
                builder: (BuildContext context, AsyncSnapshot<List<Map<String, dynamic>>> snapshot) {
                  if(snapshot.hasData){
                    ...
                    return ConstrainedBox(
                      ...
                      child: _FoundWords(
                          updateSearchWord: updateSearchWord, addSelectedWordToUnit: addSelectedWordToUnit,
                          hskFuture: hskFuture, searchWordKey: searchWordKey, units: units,
                          updateUnitNumber: updateUnitNumber, textController: textController,
                      ),
                    );
                  }else{
...
class _FoundWords extends StatelessWidget {
  ...
  const _FoundWords({required this.updateSearchWord, required this.hskFuture, required this.addSelectedWordToUnit, required this.searchWordKey, required this.units, required this.updateUnitNumber, required this.textController,});
                        ...
                        Expanded(
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              return _FoundWordsItem(index: index, hskList: hskList[index], onClick: addSelectedWordToUnit);
                            },
                          ),
                        ),
                      ],
...
class _FoundWordsItem extends StatelessWidget {
  ...
  const _FoundWordsItem({required this.index, required this.hskList, required this.onClick,});
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        overflow: TextOverflow.ellipsis,
                        style: const TextStyle(fontSize: 14, color: Color(0xFF999EA3),),
                      ),
                    ],
                  ),
                ),
